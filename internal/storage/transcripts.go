...
func (s *Store) InsertSegments(ctx context.Context, segments []TranscriptSegment) error {
	...
	for _, seg := range segments {
		_, err := tx.Exec(ctx,
			`INSERT INTO transcript_segments (session_id, user_id, character_name, start_time, end_time, text)
			 VALUES ($1, $2, $3, $4, $5, $6)`,
			seg.SessionID, seg.UserID, seg.CharacterName, seg.StartTime, seg.EndTime, seg.Text,
		)
		if err != nil {
...
func (s *Store) GetTranscript(ctx context.Context, sessionID int64) ([]TranscriptSegment, error) {
	rows, err := s.Pool.Query(ctx,
		`SELECT id, session_id, user_id, character_name, start_time, end_time, text, created_at
	...
	for rows.Next() {
		...
		err := rows.Scan(&seg.ID, &seg.SessionID, &seg.UserID, &seg.CharacterName,
