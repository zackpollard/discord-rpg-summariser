<script lang="ts">
	...
	const errorMessages: Record<string, string> = {
		...
		not_member: 'You must be a member of the Discord server to access this site.'
	};

	const errorMessage = $derived(errorParam ? errorMessages[errorParam] || 'An error occurred. Please try again.' : null);
</script>

<svelte:head>
	<title>Login - RPG Summariser</title>
</svelte:head>
...
<div class="login-page">
	<div class="login-card">
		<div class="login-brand">
			...
			<h1>RPG Summariser</h1>
		</div>

		<p class="login-desc">Sign in with your Discord account to access the session dashboard.</p>

		{#if errorMessage}
			<div class="error-box">{errorMessage}</div>
		{/if}

		<a href="/api/auth/login" class="discord-btn">
